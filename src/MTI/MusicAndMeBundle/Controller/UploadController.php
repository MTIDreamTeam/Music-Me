...
public function parseZip($file, $loc)
{
	...
	public function saveZik($zik, $save)
	{
		...
		if ($zik->title == null)
		{
			$this->err[] = "Titre : $zik->title | Erreur dans le parsage des tags ID3";
			return;
		}
		...
		if (!$musique) {
			...
			$this->err[] = "Titre : $zik->title | La musique a bien été ajouté";
		}
		else {
			if (!$save)
			{
				$this->err[] = "Titre : $zik->title | La musique éxiste déjà dans la base";
				$zik->destroy();
			}
		}
